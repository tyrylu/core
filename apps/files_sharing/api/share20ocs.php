	...
	public function createShare() {
		...
		if ($shareType === \OCP\Share::SHARE_TYPE_USER) {
			...
			if ($publicUpload === 'true') {
				...
				if ($path instanceof \OCP\Files\File) {
					return new \OC_OCS_Result(null, 404, $this->l->t('Public upload is only possible for public shared folders'));
				}
			...
			if ($expireDate !== '') {
				try {
					...
					return new \OC_OCS_Result(null, 404, $this->l->t('Invalid Date, format must be YYYY-MM-DD'));
				}
			}

		} else if ($shareType === \OCP\Share::SHARE_TYPE_REMOTE) {
			if (!$this->shareManager->outgoingServer2ServerSharesAllowed()) {
				return new \OC_OCS_Result(null, 403, $this->l->t('Sharing %s failed, because the backend does not allow shares from type %s', [$path->getPath(), $shareType]));
			}
	...
	public function updateShare($id) {
		...
		if ($share->getShareType() === \OCP\Share::SHARE_TYPE_LINK) {
			...
			if ($newPermissions !== null &&
				$newPermissions !== \OCP\Constants::PERMISSION_READ &&
				$newPermissions !== (\OCP\Constants::PERMISSION_READ | \OCP\Constants::PERMISSION_CREATE | \OCP\Constants::PERMISSION_UPDATE)) {
				return new \OC_OCS_Result(null, 400, $this->l->t('Can\'t change permission for public link share'));
			}
			...
			if ($newPermissions === (\OCP\Constants::PERMISSION_READ | \OCP\Constants::PERMISSION_CREATE | \OCP\Constants::PERMISSION_UPDATE)) {
				...
				if (!($share->getNode() instanceof \OCP\Files\Folder)) {
					return new \OC_OCS_Result(null, 400, $this->l->t('Public upload is only possible for public shared folders'));
				}
			}
